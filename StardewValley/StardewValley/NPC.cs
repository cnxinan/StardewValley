...
public class NPC : Character, IComparable
{
	...
	public virtual void ChooseAppearance(LocalizedContentManager content = null)
	{
		...
		if (location.TryGetMapProperty("UniquePortrait", out var uniquePortraitsProperty) && ArgUtility.SplitBySpace(uniquePortraitsProperty).Contains(base.Name))
		{
			...
			appliedLegacyUniquePortraits = TryLoadPortraits(assetName, out var errorPhrase7, content);
		...
		if (location.TryGetMapProperty("UniqueSprite", out var uniqueSpritesProperty) && ArgUtility.SplitBySpace(uniqueSpritesProperty).Contains(base.Name))
		{
			...
			appliedLegacyUniqueSprites = TryLoadSprites(assetName2, out var errorPhrase8, content);
		...
		if (!appliedLegacyUniquePortraits)
		{
			...
			{
				loaded2 = TryLoadPortraits(appearance.Portrait, out var errorPhrase6, content);
				if (!loaded2)
				{
			...
			if (!loaded2 && isWearingIslandAttire)
			{
				...
				{
					loaded2 = TryLoadPortraits(beachAsset2, out var errorPhrase5, content);
					if (!loaded2)
					{
			...
			if (!loaded2 && !TryLoadPortraits(defaultAsset2, out var errorPhrase4, content))
			{
		...
		if (!appliedLegacyUniqueSprites)
		{
			...
			{
				loaded = TryLoadSprites(appearance.Sprite, out var errorPhrase3, content);
				if (!loaded)
				{
			...
			if (!loaded && isWearingIslandAttire)
			{
				...
				{
					loaded = TryLoadSprites(beachAsset, out var errorPhrase2, content);
					if (!loaded)
					{
			...
			if (!loaded && !TryLoadSprites(defaultAsset, out var errorPhrase, content))
			{
	...
	public Dialogue tryToRetrieveDialogue(string preface, int heartLevel, string appendToEnd = "")
	{
		...
		for (int hearts = 10; hearts >= 2; hearts -= 2)
		{
			...
			{
				Dialogue dialogue2 = TryGetDialogue(preface + day_name + hearts + "_" + year + appendToEnd) ?? TryGetDialogue(preface + day_name + hearts + appendToEnd);
				if (dialogue2 != null)
				{
		...
		Dialogue dialogue = TryGetDialogue(preface + day_name + appendToEnd);
		if (dialogue != null)
		{
	...
	private void setNewDialogue(string dialogueSheetName, string dialogueSheetKey, bool clearOnMovement = false)
	{
		...
		if (dialogueSheetName.Contains("Marriage"))
		{
			...
			{
				Dialogue dialogue = tryToGetMarriageSpecificDialogue(dialogueSheetKey + nameToAppend) ?? new Dialogue(this, null, "");
				dialogue.removeOnNextMove = clearOnMovement;
				CurrentDialogue.Push(dialogue);
			}
			return;
