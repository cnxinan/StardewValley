...
[InstanceStatics]
[NotImplicitNetField]
public class GameLocation : INetObject<NetFields>, IEquatable<GameLocation>,  IHaveModData
{
				...
				if (this is Farm || HasMapPropertyWithValue("SpawnRandomGrassOnNewYear"))
				{
					for (int i = 0; i < 15; i++)
					{
						...
						if (o2 == null && doesTileHaveProperty(xCoord, yCoord, "Diggable", "Back") != null && !IsNoSpawnTile(location2) && isTileLocationOpen(new Location(xCoord, yCoord)) && !IsTileOccupiedBy(location2) && !isWaterTile(xCoord, yCoord))
						{
							...
							terrainFeatures.Add(location2, new Grass(grassType, 4));
						}
					}
					growWeedGrass(40);
				}
	...
	public virtual void monsterDrop(Monster monster, int x, int y, Farmer who)
	{
		...
		if (who.stats.Get("Book_Void") != 0 && Game1.random.NextDouble() < 0.03 && debrisToAdd != null && monster != null)
		{
			foreach (Debris d in debrisToAdd)
			{
				...
				{
					Item tmp2 = ItemRegistry.Create(d.itemId);
	...
	public bool damageMonster(Microsoft.Xna.Framework.Rectangle areaOfEffect, int minDamage, int maxDamage, bool isBomb, float knockBackModifier, int addedPrecision, float critChance, float critMultiplier, bool triggerMonsterInvincibleTimer, Farmer who, bool isProjectile = false)
	{
		...
		for (int i = characters.Count - 1; i >= 0; i--)
		{
			if (i < characters.Count && characters[i] is Monster { IsMonster: not false, Health: >0 } monster && monster.TakesDamageFromHitbox(areaOfEffect))
			{
	...
	protected virtual void resetLocalState()
	{
		...
		if (!ignoreLights)
		{
			...
			for (int l = 0; l < lights.Length; l += 3)
			{
				...
				{
					LogMapPropertyError("Light", lights, error4);
				}
				else
				{
			...
			if (!Game1.isTimeToTurnOffLighting(this) && !Game1.isRaining)
			{
				...
				for (int k = 0; k < windowLights.Length; k += 3)
				{
					...
					{
						LogMapPropertyError("WindowLight", windowLights, error3);
					}
					else
					{
		...
		if ((bool)isOutdoors || (bool)treatAsOutdoors)
		{
			...
			for (int j = 0; j < sounds.Length; j += 3)
			{
				...
				{
					LogMapPropertyError("BrookSounds", sounds, error2);
				}
				else
				{
	...
	public virtual bool LowPriorityLeftClick(int x, int y, Farmer who)
	{
		...
		{
			Furniture furnitureItem2 = furniture[j];
	...
	public virtual bool performAction(string[] action, Farmer who, Location tileLocation)
	{
		...
		if (who.IsLocalPlayer)
		{
					...
					if (who?.ActiveObject?.QualifiedItemId == "(O)103")
					{
						...
						tempSprite2.reachedStopCoordinate = delegate
						{
							...
							DelayedAction.functionAfterDelay(delegate
							{
								...
								temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\terraria_cat", new Microsoft.Xna.Framework.Rectangle(128, 96, 32, 32), 70f, 4, 5, tempSprite2.position + new Vector2(-5f, -3f) * 4f, flicker: false, flipped: true, 0.99f, 0f, Color.White, 4f, 0f, 0f, 0f));
								temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\terraria_cat", new Microsoft.Xna.Framework.Rectangle(128, 96, 32, 32), 60f, 4, 5, tempSprite2.position + new Vector2(-5f, 7f) * 4f, flicker: false, flipped: false, 1f, 0f, Color.White, 4f, 0f, 0f, 0f));
								temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\terraria_cat", new Microsoft.Xna.Framework.Rectangle(134, 2, 21, 38), 9999f, 1, 1, tempSprite2.position, flicker: false, flipped: false, 0.98f, 0f, Color.White, 4f, 0f, 0f, 0f)
								{
									xPeriodic = true,
								...
								for (int m = 0; m < 13; m++)
								{
									temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\terraria_cat", new Microsoft.Xna.Framework.Rectangle(134, (m == 12) ? 54 : 41, 21, 12), 9999f, 1, 1, tempSprite2.position, flicker: false, flipped: false, 0.97f - (float)m * 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f)
									{
										xPeriodic = true,
			...
			case "EmilyRoomObject":
				if (Game1.player.eventsSeen.Contains("463391") && Game1.player.spouse != "Emily")
				{
					if (getTemporarySpriteByID(5858585) is EmilysParrot parrot2)
					{
						parrot2.doAction();
					}
				}
			...
			case "LockedDoorWarp":
			{
				...
				who.faceGeneralDirection(new Vector2(tileLocation.X, tileLocation.Y) * 64f);
				lockedDoorWarp(tile, locationName, openTime2, closeTime2, npcName3, minFriendship);
				break;
			}
			case "ConditionalDoor":
			{
	...
	{
		switch ((long)Game1.player.houseUpgradeLevel)
		{
		case 0L:
	...
	public virtual void shiftContents(int dx, int dy, Func<Vector2, object, bool> where = null)
	{
		...
		foreach (KeyValuePair<Vector2, Object> v3 in list)
		{
			if (where == null || where(v3.Key, v3.Value))
			{
				...
				{
					removeLightSource(v3.Value.lightSource.identifier);
				}
				Vector2 tile2 = v3.Key + offset;
				objects.Add(tile2, v3.Value);
				v3.Value.initializeLightSource(tile2);
			}
			else
			{
				objects.Add(v3.Key, v3.Value);
			}
		}
		...
		foreach (KeyValuePair<Vector2, TerrainFeature> v4 in list2)
		{
			...
			terrainFeatures.Add(tile, v4.Value);
		}
				...
				for (int j = 0; j < numberToSpawn; j++)
				{
					for (int attempt = 0; attempt < 11; attempt++)
					{
						...
						else
						{
							...
							if (dropObject(forageObj, location2 * 64f, Game1.viewport, initialPlacement: true))
							{
								numberOfSpawnedObjectsOnMap++;
								break;
							}
	...
	{
		for (int j = resourceClumps.Count - 1; j >= 0; j--)
		{
			if (resourceClumps[j].Tile.X == (float)x && resourceClumps[j].Tile.Y == (float)y)
			{
				resourceClumps.RemoveAt(j);
			}
		}
		Vector2 v = new Vector2(x, y);
		terrainFeatures.Remove(v);
	...
	public bool buildStructure(string typeId, BuildingData data, Vector2 tileLocation, Farmer who, out Building constructed, bool magicalConstruction = false, bool skipSafetyChecks = false)
	{
		...
		if (!skipSafetyChecks)
		{
			for (int y3 = 0; y3 < tilesHigh; y3++)
			{
				...
				{
					pokeTileForConstruction(new Vector2(tileLocation.X + (float)x4, tileLocation.Y + (float)y3));
				}
			}
	...
	public void updateDoors()
	{
		...
		for (int layerHeight = buildingLayer.LayerHeight; y < layerHeight; y++)
		{
			...
			for (int layerWidth = buildingLayer.LayerWidth; x < layerWidth; x++)
			{
				...
				if (tile == null || !tile.Properties.TryGetValue("Action", out var door) || !door.ToString().Contains("Warp"))
				{
					continue;
				}
	...
	private void changeMapProperties(string propertyName, string toAdd)
	{
		try
		{
			...
			else if (!oldValue.ToString().Contains(toAdd))
			{
				string newValue = new StringBuilder(oldValue.ToString()).Append(" ").Append(toAdd).ToString();
