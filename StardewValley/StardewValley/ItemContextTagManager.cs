...
public static class ItemContextTagManager
{
	...
	public static HashSet<string> GetBaseContextTags(string itemId)
	{
		...
		if (!BaseTagsCache.TryGetValue(itemData.QualifiedItemId, out var tags))
		{
			...
			case "(O)":
			{
				if (!(itemData.RawData is ObjectData objectData1))
				{
					break;
				}
				List<string> contextTags = objectData1.ContextTags;
				if (contextTags != null && contextTags.Count > 0)
				{
					foreach (string tag in objectData1.ContextTags)
					{
						tags.Add(tag);
					}
				}
				if (!objectData1.GeodeDropsDefaultItems)
				{
					List<ObjectGeodeDropData> geodeDrops = objectData1.GeodeDrops;
