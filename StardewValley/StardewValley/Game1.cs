...
public class Game1 : InstanceGame
{
    ...
    public virtual void SetWindowSize(int w, int h)
    {
        ...
        if (activeClickableMenu is GameMenu gameMenu2)
        {
            if (gameMenu2.GetCurrentPage() is OptionsPage optionsPage)
            {
                optionsPage.preWindowSizeChange();
            }
            GameMenu gameMenu = (GameMenu)(activeClickableMenu = new GameMenu(gameMenu2.currentTab));
                    ...
                    else
                    {
                        ...
                        if (base.IsActive)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    if (!oldKBState.IsKeyDown(j) && currentMinigame != null)
                                    {
                                        currentMinigame.receiveKeyPress(j);
                                    }
                                }
                                ...
                                {
                                    if (!currentKBState.IsKeyDown(i) && currentMinigame != null)
                                    {
                                        currentMinigame.receiveKeyRelease(i);
                                    }
                                }
                                if (options.gamepadControls)
                                {
    ...
    public static void UpdateGameClock(GameTime time)
    {
        ...
        if (timeOfDay >= getTrulyDarkTime(currentLocation))
        {
            ...
            float transparency2 = Math.Min(0.93f, 0.75f + ((float)(adjustedTime2 - getTrulyDarkTime(currentLocation)) + (float)gameTimeInterval / (float)realMilliSecondsPerGameTenMinutes * 16.6f) * 0.000625f);
    ...
    public static void UpdateOther(GameTime time)
    {
        ...
        for (int l = uiOverlayTempSprites.Count - 1; l >= 0; l--)
        {
            if (uiOverlayTempSprites[l].update(time))
            {
                uiOverlayTempSprites.RemoveAt(l);
            }
        }
                    ...
                    case "FarmCave":
                        {
                            ...
                            if (getFarm().TryGetMapPropertyAs("FarmCaveEntry", out Point tile4, required: false))
                            {
                                tileX = tile4.X;
                                tileY = tile4.Y;
                                break;
                    ...
                    case "Forest":
                        {
                            ...
                            if (getFarm().TryGetMapPropertyAs("ForestEntry", out Point tile3, required: false))
                            {
                                tileX = tile3.X;
                                tileY = tile3.Y;
                                break;
    ...
    private void updatePanModeControls(MouseState currentMouseState, KeyboardState currentKBState)
    {
        ...
        if (currentMouseState.LeftButton == ButtonState.Pressed && oldMouseState.LeftButton == ButtonState.Released)
        {
            ...
            if (text != null && text.Length > 0)
            {
                ...
                panModeString = panModeString + currentLocation.Name + " " + x + " " + y2 + " ";
                panFacingDirectionWait = true;
                currentLocation.playTerrainSound(new Vector2(x, y2));
                debugOutput = panModeString;
            }
        }
        if (currentMouseState.RightButton == ButtonState.Pressed && oldMouseState.RightButton == ButtonState.Released)
        {
            ...
            Warp w2 = currentLocation.isCollidingWithWarpOrDoor(new Microsoft.Xna.Framework.Rectangle(x2, y, 1, 1));
            if (w2 != null)
            {
    ...
    public virtual void DrawWorld(GameTime time, RenderTarget2D target_screen)
    {
        if (hooks.OnRendering(RenderSteps.World, spriteBatch, time, target_screen))
        {
            ...
            {
                foreach (Farmer f2 in currentLocation.currentEvent.farmerActors)
                {
            ...
            if (hooks.OnRendering(RenderSteps.World_Sorted, spriteBatch, time, target_screen))
            {
                if (!currentLocation.shouldHideCharacters())
                {
                    if (CurrentEvent == null)
                    {
                        ...
                        {
                            if (!n3.swimming && !n3.HideShadow && !n3.isInvisible && checkCharacterTilesForShadowDrawFlag(n3))
                            {
                ...
                for (int l = 0; l < currentLocation.frontLayers.Count; l++)
                {
                    ...
                    currentLocation.frontLayers[l].Key.Draw(mapDisplayDevice, viewport, Location.Origin, wrapAround: false, 4, 64f + layer_sub_sort * layer2);
                }
            ...
            if (hooks.OnRendering(RenderSteps.World_AlwaysFront, spriteBatch, time, target_screen))
            {
                ...
                {
                    spriteBatch.Begin(SpriteSortMode.Texture, BlendState.AlphaBlend, SamplerState.PointClamp);
                    currentLocation.alwaysFrontLayers[k].Key.Draw(mapDisplayDevice, viewport, Location.Origin, wrapAround: false, 4, -1f);
                    spriteBatch.End();
                }
            }
                ...
                case 1:
                    {
                        ...
                        switch (farmerSprite.currentAnimationIndex)
                        {
                            case 0:
                                ...
                                {
                                    spriteBatch.Draw(spritesheet, new Vector2(fPosition.X - 64f + (float)toolXOffset, fPosition.Y - 160f + (float)toolYOffset), sourceRectangleForTool, color3, 0f, Vector2.Zero, 4f, SpriteEffects.None, layerDepth);
                                }
                                break;
                            case 1:
                                spriteBatch.Draw(spritesheet, new Vector2(fPosition.X - 64f + (float)toolXOffset, fPosition.Y - 160f + 8f + (float)toolYOffset), sourceRectangleForTool, color3, 0f, Vector2.Zero, 4f, SpriteEffects.None, layerDepth);
                                break;
                            case 2:
                                spriteBatch.Draw(spritesheet, new Vector2(fPosition.X - 96f + 32f + (float)toolXOffset, fPosition.Y - 128f - 24f + (float)toolYOffset), sourceRectangleForTool, color3, 0f, Vector2.Zero, 4f, SpriteEffects.None, layerDepth);
                                break;
                            case 3:
                                spriteBatch.Draw(spritesheet, new Vector2(fPosition.X - 96f + 24f + (float)toolXOffset, fPosition.Y - 128f - 32f + (float)toolYOffset), sourceRectangleForTool, color3, 0f, Vector2.Zero, 4f, SpriteEffects.None, layerDepth);
                                break;
                            case 4:
                                ...
                                {
                                    spriteBatch.Draw(spritesheet, new Vector2(fPosition.X - 64f + (float)toolXOffset, fPosition.Y - 160f + 4f + (float)toolYOffset), sourceRectangleForTool, color3, 0f, Vector2.Zero, 4f, SpriteEffects.None, layerDepth);
                                }
                                break;
                            case 5:
                                spriteBatch.Draw(spritesheet, new Vector2(fPosition.X - 64f + (float)toolXOffset, fPosition.Y - 160f + (float)toolYOffset), sourceRectangleForTool, color3, 0f, Vector2.Zero, 4f, SpriteEffects.None, layerDepth);
