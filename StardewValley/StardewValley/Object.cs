...
public class Object : Item
{
	...
	public virtual bool performToolAction(Tool t)
	{
		...
		if (base.QualifiedItemId == "(BC)165" && heldObject.Value is Chest chest2 && !chest2.isEmpty())
		{
			chest2.clearNulls();
	...
	public virtual void drawAsProp(SpriteBatch b)
	{
		...
		else
		{
			...
			b.Draw(itemData2.GetTexture(), Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + 32, y * 64 + 32)), itemData2.GetSourceRect(), Color.White, 0f, new Vector2(8f, 8f), (scale.Y > 1f) ? getScale().Y : 4f, flipped ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (float)bounds.Bottom / 10000f);
		}
	}
	...
	public override bool canBePlacedHere(GameLocation l, Vector2 tile, CollisionMask collisionMask = CollisionMask.All, bool showError = false)
	{
		...
		if (IsTapper() && l.terrainFeatures.TryGetValue(tile, out var terrainFeature2) && terrainFeature2 is Tree tree2 && !l.objects.ContainsKey(tile) && (tree2.GetData()?.CanBeTapped() ?? false))
		{
			return true;
		}
		if (base.QualifiedItemId == "(O)805" && l.terrainFeatures.TryGetValue(tile, out var terrainFeature3) && terrainFeature3 is Tree)
		{
			return true;
		}
		if (base.QualifiedItemId == "(O)419")
		{
			if (l.terrainFeatures.TryGetValue(tile, out var terrainFeature) && terrainFeature is Tree tree && !tree.stopGrowingMoss.Value)
			{
				return true;
			}
			return false;
	...
	public virtual bool placementAction(GameLocation location, int x, int y, Farmer who = null)
	{
		...
		if (!bigCraftable && !(this is Furniture))
		{
			...
			case "(O)805":
			{
				if (location.terrainFeatures.TryGetValue(placementTile, out var terrainFeature5) && terrainFeature5 is Tree tree2)
				{
			...
			case "(O)419":
			{
				if (location.terrainFeatures.TryGetValue(placementTile, out var terrainFeature4) && terrainFeature4 is Tree tree && !tree.stopGrowingMoss.Value)
				{
		...
		else
		{
			if (IsTapper())
			{
				if (location.terrainFeatures.TryGetValue(placementTile, out var terrainFeature6) && terrainFeature6 is Tree tree3 && (int)tree3.growthStage >= 5 && !tree3.stump && !location.objects.ContainsKey(placementTile) && (!tree3.isTemporaryGreenRainTree || Game1.season != Season.Summer))
				{
