...
public class Building : INetObject<NetFields>, IHaveModData
{
	...
	public virtual void performActionOnBuildingPlacement()
	{
		...
		for (int y = 0; y < (int)tilesHigh; y++)
		{
			...
			{
				Vector2 currentGlobalTilePosition2 = new Vector2((int)tileX + x, (int)tileY + y);
				if (!location.terrainFeatures.ContainsKey(currentGlobalTilePosition2) || !(location.terrainFeatures[currentGlobalTilePosition2] is Flooring) || GetData() == null || !GetData().AllowsFlooringUnderneath)
				{
	...
	public virtual void performActionOnConstruction(GameLocation location, Farmer who)
	{
		...
		else
		{
			...
			for (int bottomEdge = mainSourceRect.Height / 16 * 2; y2 <= bottomEdge; y2++)
			{
				...
				{
					location.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(666, 1851, 8, 8), 40f, 4, 2, new Vector2((int)tileX, (int)tileY) * 64f + new Vector2(x2 * 64 / 2, y2 * 64 / 2 - mainSourceRect.Height * 4 + (int)tilesHigh * 64) + new Vector2(Game1.random.Next(-32, 32), Game1.random.Next(-32, 32)), flicker: false, flipped: false)
					{
						layerDepth = (float)(((int)tileY + (int)tilesHigh) * 64) / 10000f + (float)x2 / 10000f,
						pingPong = true,
					...
					location.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(666, 1851, 8, 8), 40f, 4, 2, new Vector2((int)tileX, (int)tileY) * 64f + new Vector2(x2 * 64 / 2, y2 * 64 / 2 - mainSourceRect.Height * 4 + (int)tilesHigh * 64) + new Vector2(Game1.random.Next(-32, 32), Game1.random.Next(-32, 32)), flicker: false, flipped: false)
					{
						layerDepth = (float)(((int)tileY + (int)tilesHigh) * 64) / 10000f + (float)x2 / 10000f + 0.0001f,
						pingPong = true,
						delayBeforeAnimationStart = (mainSourceRect.Height / 16 * 2 - y2) * 100,
						scale = 4f,
						alphaFade = 0.01f,
	...
	public virtual void drawInConstruction(SpriteBatch b)
	{
		...
		if ((bool)magical || (int)daysOfConstructionLeft <= 0)
		{
			...
			{
				b.Draw(Game1.animations, Game1.GlobalToLocal(Game1.viewport, new Vector2((int)tileX * 64 - 16 + j * 16, (float)((int)tileY * 64 - mainSourceRect.Height * 4 + (int)tilesHigh * 64) + (float)(mainSourceRect.Height * 4) * (1f - drawPercentageReal))) + new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2) - ((j % 2 == 0) ? 32 : 8)), new Microsoft.Xna.Framework.Rectangle(((int)newConstructionTimer + j * 20) % 304 / 38 * 64, 768, 64, 64), Color.White * alpha * ((float)(int)newConstructionTimer / 500f), 0f, new Vector2(0f, 0f), 1f, SpriteEffects.None, (float)(((int)tileY + (int)tilesHigh) * 64) / 10000f + 0.0001f);
				if (j % 2 == 0)
				{
					b.Draw(Game1.animations, Game1.GlobalToLocal(Game1.viewport, new Vector2((int)tileX * 64 - 16 + j * 16, (float)((int)tileY * 64 - mainSourceRect.Height * 4 + (int)tilesHigh * 64) + (float)(mainSourceRect.Height * 4) * (1f - drawPercentageReal))) + new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2) - ((j % 2 == 0) ? 32 : 8)), new Microsoft.Xna.Framework.Rectangle(((int)newConstructionTimer + j * 20) % 400 / 50 * 64, 2944, 64, 64), Color.White * alpha * ((float)(int)newConstructionTimer / 500f), 0f, new Vector2(0f, 0f), 1f, SpriteEffects.None, (float)(((int)tileY + (int)tilesHigh) * 64) / 10000f + 0.0001f);
				}
			}
			return;
		}
