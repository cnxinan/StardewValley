...
public class Chest : Object
{
	...
	public bool TryMoveToSafePosition(int? preferDirection = null)
	{
		...
		bool TryMoveRecursively(Vector2 tile_position, int depth, Vector2? prioritize_direction)
		{
			...
			foreach (Vector2 offset2 in offsets)
			{
				...
				if (canBePlacedHere(location, new_position2) && location.CanItemBePlacedHere(new_position2))
				{
					if (location.objects.ContainsKey(TileLocation) && !location.objects.ContainsKey(new_position2))
					{
						location.objects.Remove(TileLocation);
						kickStartTile.Value = TileLocation;
						TileLocation = new_position2;
	...
	public override void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
	{
		...
		if ((bool)playerChest)
		{
			...
			spriteBatch.Draw(texture3, Game1.GlobalToLocal(Game1.viewport, new Vector2(draw_x * 64f + (float)((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), (draw_y - 1f) * 64f)), itemData2.GetSourceRect(), tint.Value * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, base_sort_order);
			spriteBatch.Draw(texture3, Game1.GlobalToLocal(Game1.viewport, new Vector2(draw_x * 64f + (float)((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), (draw_y - 1f) * 64f)), itemData2.GetSourceRect(0, currentLidFrame), tint.Value * alpha * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, base_sort_order + 1E-05f);
			return;
		}
		if ((bool)giftbox)
		{
