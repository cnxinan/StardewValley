	...
	public static int positionOfNextSpace(string s, int index, int currentXPosition, int accumulatedHorizontalSpaceBetweenCharacters)
	{
		...
		for (int i = index; i < s.Length; i++)
		{
			if (isUsingNonSpriteSheetFont())
			{
				...
				currentXPosition = ((!characterMap.TryGetValue(s[i], out var fc2)) ? (currentXPosition + (int)((float)FontFile.Common.LineHeight * FontPixelZoom)) : (currentXPosition + (int)((float)fc2.XAdvance * FontPixelZoom)));
				continue;
			}
			if (s[i] == ' ' || s[i] == '^')
			{
