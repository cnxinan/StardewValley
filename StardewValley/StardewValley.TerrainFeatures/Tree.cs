...
public class Tree : TerrainFeature
{
	...
	public override bool performToolAction(Tool t, int explosion, Vector2 tileLocation)
	{
		...
		if ((int)growthStage >= 5)
		{
			...
			else
			{
				...
				damage = (long)t.upgradeLevel switch
				{
					0L => 1f, 
					1L => 1.25f, 
					2L => 1.67f, 
		...
		else if ((int)growthStage >= 3)
		{
			...
			shake(tileLocation, doEvenIfStillShaking: true);
			float damage2 = 1f;
			damage2 = ((explosion > 0) ? ((float)explosion) : ((long)t.upgradeLevel switch
			{
				0L => 2f, 
				1L => 2.5f, 
				2L => 3.34f, 
			...
			if (data != null)
			{
				...
				else if (Game1.IsMultiplayer)
				{
					...
					if (chopItems != null && chopItems.Count > 0)
					{
						...
						foreach (WildTreeChopItemData drop in data.ChopItems)
						{
							Item item = TryGetDrop(drop, r, targetFarmer2, "ChopItems");
							if (item != null)
							{
				...
				else
				{
					...
					if (chopItems2 != null && chopItems2.Count > 0)
					{
						...
						{
							Item item2 = TryGetDrop(drop2, r, targetFarmer, "ChopItems");
							if (item2 != null)
							{
	...
	public override void drawInMenu(SpriteBatch spriteBatch, Vector2 positionOnScreen, Vector2 tileLocation, float scale, float layerDepth)
	{
		...
		if ((int)growthStage < 5)
		{
			Microsoft.Xna.Framework.Rectangle sourceRect = (long)growthStage switch
			{
