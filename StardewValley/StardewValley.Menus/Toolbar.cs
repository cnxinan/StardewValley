...
public class Toolbar : IClickableMenu
{
	...
	public void shifted(bool right)
	{
		...
		{
			buttons[j].scale = 1f + (float)(11 - j) * 0.03f;
		}
	}
	...
	public override void draw(SpriteBatch b)
	{
		...
		for (int j = 0; j < 12; j++)
		{
			Vector2 toDraw2 = new Vector2(Game1.uiViewport.Width / 2 - 384 + j * 64, yPositionOnScreen - 96 + 8);
			b.Draw(Game1.menuTexture, toDraw2, Game1.getSourceRectForStandardTileSheet(Game1.menuTexture, (Game1.player.CurrentToolIndex == j) ? 56 : 10), Color.White * transparency);
			if (!Game1.options.gamepadControls)
			{
				b.DrawString(Game1.tinyFont, slotText[j], toDraw2 + new Vector2(4f, -8f), Color.DimGray * transparency);
			}
		}
		for (int i = 0; i < 12; i++)
		{
