...
public class TitleMenu : IClickableMenu, IDisposable
{
	...
	public override void receiveGamePadButton(Buttons b)
	{
		...
		if (subMenu != null)
		{
			if (subMenu is LoadGameMenu { deleteConfirmationScreen: not false })
			{
				passThrough = false;
			}
			if (subMenu is CharacterCustomization { showingCoopHelp: not false })
			{
				passThrough = false;
			}
				...
				if (Game1.content.GetCurrentLanguage() != LocalizedContentManager.LanguageCode.zh)
				{
					...
					else if (cornerRect.Contains(x, y) && !cornerPhaseHolding)
					{
						...
						else
						{
							...
							for (int j = 0; j < 3; j++)
							{
								tempSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(280 + Game1.random.Choose(8, 0), 1954, 8, 8), 1000f, 1, 99, new Vector2(width / 2 - 190 * zoom4, -300 * zoom4 - (int)(viewportY / 3f) * zoom4 + 175 * zoom4), flicker: false, flipped: false, 1f, 0f, Color.White, 3f, 0f, 0f, (float)Game1.random.Next(-10, 11) / 100f)
								{
	...
	public override void draw(SpriteBatch b)
	{
		...
		{
			for (int x3 = 0; x3 < width; x3 += 638)
			{
				b.Draw(Game1.mouseCursors, new Vector2(x3, y3) * 4f, new Rectangle(0, 1453, 638, 195), Color.White * ((float)globalXOffset / 1200f), 0f, Vector2.Zero, 4f, effect, 0.8f);
			}
		...
		else if (logoFadeTimer > 0 || fadeFromWhiteTimer > 0)
		{
			...
			if (specialSurprised)
			{
				...
				for (int x = 64; x < width + 1000; x += 192)
				{
					for (int y = -1000; y < height; y += 192)
					{
						b.Draw(Game1.mouseCursors, new Vector2(x, y) + new Vector2((float)(-time) / 20f, (float)time / 20f), new Rectangle(355 + (time + x * 77 + y * 77) / 12 % 110 / 11 * 16, 1200, 16, 16), Color.White * 0.66f * ((float)(fadeFromWhiteTimer - (2000 - fadeFromWhiteTimer)) / 2000f), 0f, Vector2.Zero, 3f, SpriteEffects.None, 0.18f);
					}
				}
				...
				Utility.drawWithShadow(b, titleButtonsTexture, new Vector2(width / 2 - 261, height / 2 - 102), new Rectangle((time / 200 % 2 == 0) ? 85 : 0, 559, 85, 69), Color.White * ((float)(fadeFromWhiteTimer - (2000 - fadeFromWhiteTimer)) / 2000f), 0f, Vector2.Zero, 3f, flipped: false, 0.2f, -4, -4, 0f);
			}
