...
public class ShopMenu : IClickableMenu
{
	...
	public override void draw(SpriteBatch b)
	{
		...
		for (int k = 0; k < forSaleButtons.Count; k++)
		{
			...
			bool failedCanPurchaseCheck = canPurchaseCheck != null && !canPurchaseCheck(currentItemIndex + k);
			IClickableMenu.drawTextureBox(b, theme.ItemRowBackgroundTexture, theme.ItemRowBackgroundSourceRect, forSaleButtons[k].bounds.X, forSaleButtons[k].bounds.Y, forSaleButtons[k].bounds.Width, forSaleButtons[k].bounds.Height, (forSaleButtons[k].containsPoint(Game1.getOldMouseX(), Game1.getOldMouseY()) && !scrolling) ? theme.ItemRowBackgroundHoverColor : Color.White, 4f, drawShadow: false);
			...
			{
				b.Draw(theme.ItemIconBackgroundTexture, new Vector2(forSaleButtons[k].bounds.X + 32 - 12, forSaleButtons[k].bounds.Y + 24 - 4), theme.ItemIconBackgroundSourceRect, Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
				Vector2 drawPos = new Vector2(forSaleButtons[k].bounds.X + 32 - 8, forSaleButtons[k].bounds.Y + 24);
				...
				bool hasPrice = itemPriceAndStock[forSale[currentItemIndex + k]].Price > 0;
				if (SpriteText.getWidthOfString(formattedDisplayName) > width - (hasPrice ? (150 + SpriteText.getWidthOfString(itemPriceAndStock[forSale[currentItemIndex + k]].Price + " ")) : 100) && formattedDisplayName.Length > (hasPrice ? 27 : 37))
				{
					formattedDisplayName = formattedDisplayName.Substring(0, hasPrice ? 27 : 37);
					formattedDisplayName += "...";
				}
				SpriteText.drawString(b, formattedDisplayName, forSaleButtons[k].bounds.X + 96 + 8, forSaleButtons[k].bounds.Y + 28, 999999, -1, 999999, failedCanPurchaseCheck ? 0.5f : 1f, 0.88f, junimoText: false, -1, "", theme.ItemRowTextColor);
			}
			else
			{
				SpriteText.drawString(b, displayName, forSaleButtons[k].bounds.X + 32 + 8, forSaleButtons[k].bounds.Y + 28, 999999, -1, 999999, failedCanPurchaseCheck ? 0.5f : 1f, 0.88f, junimoText: false, -1, "", theme.ItemRowTextColor);
			}
			int right = forSaleButtons[k].bounds.Right;
			int tradeIconDrawY = forSaleButtons[k].bounds.Y + 28 - 4;
			int tradeTextDrawY = forSaleButtons[k].bounds.Y + 44;
			if (itemPriceAndStock[forSale[currentItemIndex + k]].Price > 0)
			{
				SpriteText.drawString(b, itemPriceAndStock[forSale[currentItemIndex + k]].Price + " ", right - SpriteText.getWidthOfString(itemPriceAndStock[forSale[currentItemIndex + k]].Price + " ") - 60, forSaleButtons[k].bounds.Y + 28, 999999, -1, 999999, (getPlayerCurrencyAmount(Game1.player, currency) >= itemPriceAndStock[forSale[currentItemIndex + k]].Price && !failedCanPurchaseCheck) ? 1f : 0.5f, 0.88f, junimoText: false, -1, "", theme.ItemRowTextColor);
				...
				right -= SpriteText.getWidthOfString(itemPriceAndStock[forSale[currentItemIndex + k]].Price + " ") + 96;
				tradeIconDrawY = forSaleButtons[k].bounds.Y + 20;
				tradeTextDrawY = forSaleButtons[k].bounds.Y + 28;
			}
			if (itemPriceAndStock[forSale[currentItemIndex + k]].TradeItem != null)
			{
				int required_item_count = 5;
				string requiredItem = itemPriceAndStock[forSale[currentItemIndex + k]].TradeItem;
				if (requiredItem != null && itemPriceAndStock[forSale[currentItemIndex + k]].TradeItemCount.HasValue)
				{
					required_item_count = itemPriceAndStock[forSale[currentItemIndex + k]].TradeItemCount.Value;
				}
