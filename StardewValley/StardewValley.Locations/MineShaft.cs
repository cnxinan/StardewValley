				...
				for (int l = 0; l < backLayer.LayerHeight; l++)
				{
					...
					if (isTileClearForMineObjects(j, l))
					{
						...
						else if (mineRandom.NextDouble() <= monsterChance && getDistanceFromStart(j, l) > 5f)
						{
							...
							{
								monsterToAdd = BuffMonsterIfNecessary(getMonsterForThisLevel(mineLevel, j, l));
							}
						...
						else if (mineRandom.NextDouble() <= itemChance)
						{
							...
							base.Objects.Add(objectPos2, getRandomItemForThisLevel(mineLevel, objectPos2));
						}
						else if (mineRandom.NextDouble() <= 0.005 && !isDarkArea() && !mustKillAllMonstersToAdvance() && (GetAdditionalDifficulty() <= 0 || (getMineArea() == 40 && mineLevel % 40 < 30)))
						{
							...
							resourceClumps.Add(new ResourceClump(whichClump, 2, 2, objectPos3));
						}
					...
					else if (isContainerPlatform(j, l) && CanItemBePlacedHere(new Vector2(j, l)) && mineRandom.NextDouble() < 0.4 && (firstTime || canAdd(0, barrelsAdded)))
					{
						...
						objects.Add(objectPos4, BreakableContainer.GetBarrelForMines(objectPos4, this));
						barrelsAdded++;
						if (firstTime)
						{
