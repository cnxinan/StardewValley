...
public class IslandSouth : IslandLocation
{
	public class IslandActivityAssigments
	{
		...
		public virtual void FindActivityForCharacters(Dictionary<Character, string> last_activity_assignments)
		{
			...
			foreach (NPC character2 in visitors)
			{
				...
				if (!(name == "Gus"))
				{
					if (!(name == "Sam") || !TryAssignment(character2, towelLoungePoints, "Resort_Towel", character2.name.Value.ToLower() + "_beach_towel", animation_required: true, 0.5, last_activity_assignments))
					{
						continue;
					}
	...
	public static void SetupIslandSchedules()
	{
		...
		foreach (NPC visitor in visitors)
		{
			...
			if (should_dress)
			{
				Point dressing_room2 = GetDressingRoomPoint(visitor);
