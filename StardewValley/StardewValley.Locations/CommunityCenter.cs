		...
		if (restoreAreaTimer > 0)
		{
			...
			case 1:
				if (Game1.IsMasterGame && Game1.random.NextDouble() < 0.4)
				{
					...
					Junimo k = new Junimo(v * 64f, restoreAreaIndex, temporary: true);
					if (!isCollidingPosition(k.GetBoundingBox(), Game1.viewport, k))
					{
						characters.Add(k);
			...
			case 2:
				...
				if (Game1.IsMasterGame)
				{
					for (int i = characters.Count - 1; i >= 0; i--)
					{
						if (characters[i] is Junimo junimo && (bool)junimo.temporaryJunimo)
						{
							characters.RemoveAt(i);
						}
					}
