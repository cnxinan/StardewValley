...
public class VolcanoDungeon : IslandLocation
{
	...
	private int chooseStoneTypeIndexOnly(Vector2 tile)
	{
		...
		{
			if (objects.TryGetValue(v6, out var obj6) && (obj6.QualifiedItemId == "(O)843" || obj6.QualifiedItemId == "(O)844"))
			{
				chance += 0.15;
			}
		}
		...
		else
		{
			...
			{
				if (objects.TryGetValue(v5, out var obj5) && obj5.QualifiedItemId == "(O)765")
				{
					chance += 0.1;
				}
			}
			...
			else
			{
				...
				{
					if (objects.TryGetValue(v4, out var obj4) && obj4.QualifiedItemId == "(O)VolcanoGoldNode")
					{
						chance += 0.2;
					}
				}
				...
				else
				{
					...
					{
						if (objects.TryGetValue(v3, out var obj3) && obj3.QualifiedItemId.StartsWith("(O)VolcanoCoalNode"))
						{
							chance += 0.2;
						}
					}
					...
					else
					{
						...
						{
							if (objects.TryGetValue(v2, out var obj2) && obj2.QualifiedItemId == "(O)850")
							{
								chance += 0.25;
							}
						}
	...
	public virtual void GenerateWalls(Color match, int source_x, int source_y, int wall_height = 4, int random_wall_variants = 1, bool start_in_wall = false, Action<int, int> on_insufficient_wall_height = null, bool use_corner_hack = false)
	{
		...
		for (int pass = 0; pass < 2; pass++)
		{
			for (int x2 = 0; x2 < mapWidth; x2++)
			{
				...
				{
					if (GetPixel(x2, current_y, match) != match || current_y >= mapHeight)
					{
						...
						for (int curr_y = current_y - 1; curr_y > last_y; curr_y--)
						{
							if (clearance < wall_height)
							{
								...
								{
									SetPixelMap(x2, curr_y, Color.White);
									PlaceSingleWall(x2, curr_y);
								}
								current_height--;
							}
							else
							{
								...
								{
								case 0:
									if (GetPixelClearance(x2 - 1, curr_y, wall_height, match) < wall_height && GetPixelClearance(x2 + 1, curr_y, wall_height, match) < wall_height)
									{
										if (on_insufficient_wall_height != null)
										{
											on_insufficient_wall_height(x2, curr_y);
									...
									break;
								case 1:
									heightMap[x2 + curr_y * mapWidth] = current_height + 1;
									if (current_height < wall_height || wall_height == 0)
									{
										if (wall_height > 0)
										{
											SetTile(buildingsLayer, x2, curr_y, GetTileIndex(source_x + random_wall_variants + wall_variant_index, source_y + 1 + random_wall_variants + wall_height - current_height - 1));
										}
									}
									else
									{
										SetTile(buildingsLayer, x2, curr_y, GetTileIndex(source_x + random_wall_variants * 3, source_y));
									}
									break;
								}
							}
		...
		for (int y = 0; y < mapHeight; y++)
		{
			for (int x = 0; x < mapWidth; x++)
			{
				int height = GetHeight(x, y, wall_height);
				int left_height = GetHeight(x - 1, y, wall_height);
				int right_height = GetHeight(x + 1, y, wall_height);
				int top_height = GetHeight(x, y - 1, wall_height);
				...
				if (right_height < height)
				{
					if (right_height == wall_height)
					{
						if (use_corner_hack)
						{
							...
							SetTile(buildingsLayer, x, y, GetTileIndex(source_x + random_wall_variants * 3, source_y));
						}
						else
						{
							SetTile(buildingsLayer, x, y, GetTileIndex(source_x + random_wall_variants * 3, source_y + 1));
						}
					}
					else
					{
						...
						if (right_height >= 0)
						{
							SetTile(buildingsLayer, x, y, GetTileIndex(source_x + random_wall_variants, source_y + 1 + random_wall_variants + wall_height - right_height));
							target_layer = frontLayer;
						}
						if (height > wall_height)
						{
							SetTile(target_layer, x, y, GetTileIndex(source_x + random_wall_variants * 3 - 1, source_y + 1 + index));
						}
						else
						{
							SetTile(target_layer, x, y, GetTileIndex(source_x + random_wall_variants * 2 + index, source_y + 1 + random_wall_variants * 2 + 1 - height - 1));
						}
						if (wall_height > 0 && y + 1 < mapHeight && right_height == -1 && GetHeight(x + 1, y + 1, wall_height) >= 0 && GetHeight(x, y + 1, wall_height) >= 0)
						{
							if (use_corner_hack)
							{
								corner_tiles.Add(new Point(x, y));
							...
							{
								SetTile(frontLayer, x, y, GetTileIndex(source_x + random_wall_variants * 3, source_y + 2));
							}
						}
					}
				}
				else if (left_height < height)
				{
					if (left_height == wall_height)
					{
						...
						{
							SetTile(buildingsLayer, x, y, GetTileIndex(source_x + random_wall_variants * 3 + 1, source_y + 1));
						}
					}
					else
					{
						...
						if (left_height >= 0)
						{
							SetTile(buildingsLayer, x, y, GetTileIndex(source_x + random_wall_variants, source_y + 1 + random_wall_variants + wall_height - left_height));
							target_layer2 = frontLayer;
						}
						if (height > wall_height)
						{
							SetTile(target_layer2, x, y, GetTileIndex(source_x, source_y + 1 + index));
						}
						else
						{
							SetTile(target_layer2, x, y, GetTileIndex(source_x + index, source_y + 1 + random_wall_variants * 2 + 1 - height - 1));
						}
						if (wall_height > 0 && y + 1 < mapHeight && left_height == -1 && GetHeight(x - 1, y + 1, wall_height) >= 0 && GetHeight(x, y + 1, wall_height) >= 0)
						{
							if (use_corner_hack)
							{
								corner_tiles.Add(new Point(x, y));
							...
							{
								SetTile(frontLayer, x, y, GetTileIndex(source_x + random_wall_variants * 3 + 1, source_y + 2));
							}
						}
					}
				}
				...
				if (wall_height > 0)
				{
					if (right_height == -1)
					{
						SetTile(frontLayer, x, y - 1, GetTileIndex(source_x + random_wall_variants * 2 + index, source_y));
					}
					else if (left_height == -1)
					{
						SetTile(frontLayer, x, y - 1, GetTileIndex(source_x + index, source_y));
					}
					else
					{
						SetTile(frontLayer, x, y - 1, GetTileIndex(source_x + random_wall_variants + index, source_y));
					}
				}
				else if (right_height == -1)
				{
					SetTile(buildingsLayer, x, y, GetTileIndex(source_x + random_wall_variants * 2 + index, source_y));
				}
				else if (left_height == -1)
				{
					SetTile(buildingsLayer, x, y, GetTileIndex(source_x + index, source_y));
				}
				else
				{
					SetTile(buildingsLayer, x, y, GetTileIndex(source_x + random_wall_variants + index, source_y));
				}
			}
		}
		if (use_corner_hack)
