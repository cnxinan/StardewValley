	...
	public virtual void ReadWallpaperAndFloorTileData()
	{
		...
		else
		{
			for (int x2 = 0; x2 < map.Layers[0].LayerWidth; x2++)
			{
				...
				{
					string tile_property2 = doesTileHaveProperty(x2, y2, "WallID", "Back");
					getTileIndexAt(new Point(x2, y2), "Back");
					if (tile_property2 == null)
					{
						continue;
					}
					if (!wallpaperIDs.Contains(tile_property2))
					{
						wallpaperIDs.Add(tile_property2);
					}
					if (appliedWallpaper.TryAdd(tile_property2, "0") && initial_values.TryGetValue(tile_property2, out var initial_value2))
					{
						if (appliedWallpaper.TryGetValue(initial_value2, out var newValue2))
						{
							appliedWallpaper[tile_property2] = newValue2;
						}
						else if (GetWallpaperSource(initial_value2).Value >= 0)
						{
							appliedWallpaper[tile_property2] = initial_value2;
						}
					}
					if (!wallpaperTiles.TryGetValue(tile_property2, out var areas2))
					{
						areas2 = (wallpaperTiles[tile_property2] = new List<Vector3>());
					}
					areas2.Add(new Vector3(x2, y2, 0f));
					string tilesheet_id = getTileSheetIDAt(x2, y2, "Back");
					map.GetTileSheet(tilesheet_id);
					if (IsFloorableOrWallpaperableTile(x2, y2 + 1, "Back"))
					{
						areas2.Add(new Vector3(x2, y2 + 1, 1f));
					}
					if (IsFloorableOrWallpaperableTile(x2, y2 + 2, "Buildings"))
					{
						areas2.Add(new Vector3(x2, y2 + 2, 2f));
					}
					else if (IsFloorableOrWallpaperableTile(x2, y2 + 2, "Back") && !IsFloorableTile(x2, y2 + 2, "Back"))
					{
						areas2.Add(new Vector3(x2, y2 + 2, 2f));
					}
				}
	...
	public override void TransferDataFromSavedLocation(GameLocation l)
	{
		if (l is DecoratableLocation decoratable_location)
		{
			...
			else
			{
				...
				{
					appliedWallpaper[key4] = decoratable_location.appliedWallpaper[key4];
				}
				foreach (string key3 in decoratable_location.appliedFloor.Keys)
				{
					appliedFloor[key3] = decoratable_location.appliedFloor[key3];
				}
			}
		}
		setWallpapers();
