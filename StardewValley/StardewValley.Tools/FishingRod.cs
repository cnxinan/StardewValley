...
public class FishingRod : Tool
{
	...
	{
		return (long)upgradeLevel switch
		{
			0L => Color.Goldenrod, 
			1L => Color.OliveDrab, 
			2L => Color.White, 
	...
	public override void tickUpdate(GameTime time, Farmer who)
	{
		...
		if (castingChosenCountdown > 0f)
		{
			...
			if (castingChosenCountdown <= 0f && who.CurrentTool != null)
			{
				...
				if (who.FacingDirection == 1 || who.FacingDirection == 3)
				{
					...
					Rectangle sourceRect2 = Game1.getSourceRectForStandardTileSheet(Game1.bobbersTexture, getBobberStyle(who), 16, 32);
					sourceRect2.Height = 16;
					animations.Add(new TemporaryAnimatedSprite("TileSheets\\bobbers", sourceRect2, t2, 1, 0, who.Position + new Vector2(0f, -96f), flicker: false, flipped: false, (float)playerPixel3.Y / 10000f, 0f, Color.White, 4f, 0f, 0f, (float)Game1.random.Next(-20, 20) / 100f)
					{
		...
		else if (who.UsingTool && castedButBobberStillInAir)
		{
			...
			else
			{
				...
				for (int j = 0; j < _totalMotionBuffer.Length; j++)
				{
					applied_motion += _totalMotionBuffer[j];
				}
